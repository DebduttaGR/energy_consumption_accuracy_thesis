...
class CLISource(DataSource):
    ...
    def start(self):
        try:
            cmd = (
                'ssh debdutta@GL5 '
                '"source /home/debdutta/Experiment/py13env/bin/activate && '
                'cd /home/debdutta/Experiment/FlightDelayPrediction && '
                'energibridge --output energibridge.csv --summary '
                'python3 run_two_phase.py --threshold 1.0 > metrics.csv"'
        )

            proc = subprocess.Popen(cmd, shell=True)
            print("Started SSH process with PID:", proc.pid)
        ...
        #                                     stderr=subprocess.PIPE)
        # try:
        #     # hand the entire string to /bin/bash -lc so our embedded quotes & redirects survive
        #     self.process = subprocess.Popen(
        #         self._format_cmd(),
        #         shell=True,
        #         executable="/bin/bash",
        ...
        #     )
        # except Exception as e:
        #     # if something half‑started, clean up
        #     if getattr(self, "process", None):
        #         try:
        #             self.process.kill()
        #         except Exception:
        #             pass
        #     raise RuntimeError(f"{self.source_name} process could not start: {e!s}")
